...
public class DistanceVisionResultActivity extends AppCompatActivity {

    private static final String TAG = "DistanceVisionResult";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_distance_vision_result);
        ...
        testManager.setDistanceVisionCompleted(true);
        testManager.setDistanceVisionScores(rightEyeScore, leftEyeScore);
    ...
    private void saveToDatabase() {
        int profileId = sessionManager.getSelectedProfileId();
        Log.d(TAG, "saveToDatabase called - profileId: " + profileId);

        if (profileId <= 0) {
            Log.w(TAG, "No profile selected (profileId=" + profileId + "), skipping database save");
            return;
        }
        ...
        Log.d(TAG, "Saving test result - profileId: " + profileId + ", testId: 1, rightScore: " + rightPercent
                + ", leftScore: " + leftPercent);

        apiManager.saveTestResult(profileId, 1, rightPercent, leftPercent, status,
                new ApiManager.ApiCallback() {
                    @Override
                    public void onSuccess(JSONObject response) {
                        Log.d(TAG, "SUCCESS - Test result saved to database: " + response.toString());
                    }

                    @Override
                    public void onError(String errorMessage) {
                        Log.e(TAG, "ERROR - Failed to save test result: " + errorMessage);
                    }
                });
    }
    ...
    private void displayResults() {
        ...
        if (totalScore >= 9) {
            assessment = "Excellent! Your distance vision appears to be very good. Continue regular eye care practices.";
        } else if (totalScore >= 7) {
            assessment = "Good! Your distance vision is satisfactory. Regular check-ups are recommended.";
        } else if (totalScore >= 5) {
            assessment = "Fair. You may want to consider an eye examination for a more thorough evaluation.";
        } else {
            assessment = "We recommend consulting an eye care professional for a comprehensive eye examination.";
        }
